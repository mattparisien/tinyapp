...
const validateLogIn = function (object, email, password, req, res, bcrypt) {
  if (!req.body.email || !req.body.password) {
    ...
    !bcrypt.compareSync(
      password,
      fetchPassword(object, fetchUserByEmail(object, email)["id"])
    )
  ) {
...
const validateRegister = function (object, email, req, res) {
  ...
  if (!req.body.email || !req.body.password) {
    res
      .status(400)
      .redirect(`/register?error=${"Please fill out the fields."}`);
    ...
    res
      .status(400)
      .redirect(
        `/register?error=${"You already have an account with this email address."}`
      );
  }
};
